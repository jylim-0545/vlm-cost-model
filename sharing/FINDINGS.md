...
> **재현 (동봉 어댑터).** 위 표의 값은 3-seed 연구 평균이다. `sharing/adapters_pretrained/`에 담은
> recon/e2e 어댑터로 이 모듈에서 직접 재평가하면(MMStar n=400, seed 0, native 56.2 공통):
> raw **0.81**(즉석 `--mode raw`) · recon **0.87**(`--load-adapter fine_recon_s0.pt`) · e2e **0.87**
> (`fine_e2e_s0.pt`). s0 단독이라 e2e가 3-seed 평균(0.91)보다 낮게 나온다. raw/ridge는 파일 없이
> 즉석 재현되므로 동봉하지 않았다.
...
균형. → `--multitask aokvqa` + `--forget aokvqa`(망각 측정). λ=2 어댑터는
`adapters_pretrained/multitask_l2.pt`로 동봉(`--load-adapter`로 재평가).
...
InternVL-8B 변환기(1152→4096)는 `adapters_pretrained/crossbackbone_internvl8.pt`로 동봉(보관용 —
`embed` 주입이라 이 모듈의 OV 전용 `--load-adapter` 재평가로는 안 돌아간다).
